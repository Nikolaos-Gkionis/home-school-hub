...
module SidebarNavHelper
  ...
  def sidebar_subject_summary_classes(status, current: false)
    base = "flex cursor-pointer list-none items-center justify-between gap-2 rounded-md px-2 py-1.5 text-xs font-semibold marker:content-none sm:text-sm border-l-2 border-transparent"
  ...
  def sidebar_unit_summary_classes(status, current: false)
    base = "flex cursor-pointer list-none items-center justify-between gap-2 rounded-md px-2 py-1.5 pl-3 text-[11px] marker:content-none sm:text-xs border-l-2 border-transparent"
  ...
  def sidebar_status_dot(status)
    case status
    when :complete
      tag.span(class: "h-1.5 w-1.5 shrink-0 rounded-full bg-emerald-400", "aria-hidden": "true")
    when :in_progress
      tag.span(class: "h-1.5 w-1.5 shrink-0 rounded-full bg-amber-400", "aria-hidden": "true")
    else
      tag.span(class: "h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--color-border)]", "aria-hidden": "true")
    end
  end
end
